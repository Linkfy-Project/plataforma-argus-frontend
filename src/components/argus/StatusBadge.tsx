...
const map: Record<ObraStatus, string> = {
  ...
  "Atrasada": "bg-[color:var(--warning)]/15 text-[color:var(--warning)] border-[color:var(--warning)]/30",
...
export function StatusBadge({ status }: { status: ObraStatus | string }) {
  const cls = (map as Record<string, string>)[status] || "bg-muted text-muted-foreground border-border";
  return (
